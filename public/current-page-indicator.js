/* Keep the header navigation visibly synced with the current route. */
(function(){
  const update=()=>{
    ...
    document.querySelectorAll('.nav nav a').forEach(link=>{
      ...
      else if(label==='about us')active=path==='/about-us';
      link.classList.toggle('currentPage',active);
      if(active)link.setAttribute('aria-current','page');else link.removeAttribute('aria-current');
    });
    const wishlist=document.querySelector('.wishlistNav');if(wishlist)wishlist.classList.toggle('currentPage',path==='/wishlist');
    const cart=document.querySelector('.cart');if(cart)cart.classList.toggle('currentPage',path==='/cart');
    const account=document.querySelector('.accountWrap:not(.signedIn) .account');if(account)account.classList.toggle('currentPage',path==='/account');
  };
  ...
  window.addEventListener('popstate',update);window.addEventListener('motodc-routechange',update);
