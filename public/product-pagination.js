(()=>{
 ...
 const findNav=c=>c.classList.contains('adminlist')
  ?document.querySelector('.adminOrders + .productPagination[data-owner="admin"]')
 ...
 const render=(c,type)=>{
  ...
  if(!nav){
   ...
   if(type==='admin'){
    const orders=document.querySelector('.adminOrders');
    if(orders)orders.insertAdjacentElement('afterend',nav);else c.insertAdjacentElement('afterend',nav);
