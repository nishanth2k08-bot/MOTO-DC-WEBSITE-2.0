...
export default function AdminOrders({ activeTab = 'dashboard' }){
 ...
 const sendEmail=async(event,orderDocId,paymentMethod,status)=>{try{const u=adminAuth.currentUser;if(!u)throw new Error('Admin session expired');const token=await u.getIdToken();const response=await fetch('/api/send-order-email',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({event,orderDocId,paymentMethod,status})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Email could not be sent');return data}catch(e){console.error(e);toast.error(e.message||'Could not send customer email');return null}};
 ...
 return (
  <div className="adminOrders">
   {activeTab==='dashboard'&&(
    <div className="adminDash">
     <div className="ordersHead">
      <div>
       <p className="eyebrow">METRICS & OVERVIEW</p>
       <h2>Performance <span>metrics.</span></h2>
       <p>Live sales, customers, inventory and order activity.</p>
      </div>
      <div className="adminDashActions">
       <button className="adminsignout" onClick={load} disabled={loading||clearing}>
        <RefreshCw size={16}/> {loading?'Loading...':'Refresh'}
       </button>
       {orders.length>0&&<button className="adminClearOrders" onClick={clearOrders} disabled={clearing||loading}>
        <Trash2 size={16}/> {clearing?'Clearing...':'Clear Orders'}
       </button>}
      </div>
     </div>
     <div className="dashStats">
      <div><span><IndianRupee size={16}/></span><small>Revenue</small><strong>{money(stats.revenue)}</strong></div>
      <div><span><ShoppingBag size={16}/></span><small>Active orders</small><strong>{stats.activeOrders}</strong></div>
      <div><span><Users size={16}/></span><small>Customers</small><strong>{stats.customers}</strong></div>
      <div><span><Package size={16}/></span><small>Units in stock</small><strong>{stats.units}</strong></div>
     </div>
     <div className="dashGrid">
      <div className="dashPanel">
       <div className="dashPanelHead"><h3><TrendingUp size={18}/> Order status</h3></div>
       <div className="statusGrid">
        <div><b>{stats.pending}</b><small>Processing</small></div>
        <div><b>{stats.shipped}</b><small>Shipped</small></div>
        <div><b>{stats.delivered}</b><small>Delivered</small></div>
       </div>
      </div>
      <div className="dashPanel">
       <div className="dashPanelHead"><h3><AlertTriangle size={18}/> Low stock</h3></div>
       {stats.low.length?<div className="lowStock">{stats.low.slice(0,5).map(p=><div key={p.id}><span>{p.name}</span><b>{p.stock} left</b></div>)}</div>:<p className="dashMuted">All products have healthy stock levels.</p>}
      </div>
     </div>
   ...
   {activeTab==='orders'&&(
    <>
     <div className="ordersHead orderManagementHead">
      <div>
