...
async function saveUserProfile(user,name=''){
 ...
 const savedName=snap.exists()?String(snap.data()?.name||'').trim():'';
 const profileName=(name||user.displayName||savedName).trim();
...
function loadMsg91Widget(){
 return new Promise((resolve,reject)=>{
  ...
  script.async=true;
  script.dataset.msg91OtpWidget='true';
  script.onload=()=>resolve();
  script.onerror=()=>reject(new Error('MSG91 OTP Widget could not load'));
...
export default function Auth(){
 ...
 useEffect(()=>{
  ...
  loadMsg91Widget().then(()=>{
   ...
   console.error(e);
   if(alive)toast.error('Phone OTP service could not be loaded');
  });
  return()=>{alive=false};
 },[]);
 ...
 function handleOtpKeyDown(index,e){
  if(e.key==='Backspace'){
   ...
   else if(index>0){next[index-1]='';otpRefs.current[index-1]?.focus()}
   updateOtp(next);
  }
 ...
 async function social(provider){
  ...
  try{
   ...
   toast.success('Welcome back to MotoDC!');
   setTimeout(()=>nav('/',{replace:true}),350);
  }catch(e){
   console.error(e);
   const m={'auth/popup-closed-by-user':'Sign-in window was closed','auth/popup-blocked':'Your browser blocked the sign-in popup','auth/account-exists-with-different-credential':'This email is already registered with another sign-in method'};
   toast.error(m[e.code]||e.message||'Social sign-in failed');
 ...
 async function submitEmail(e){
  ...
  try{
   let cred;
   if(mode==='signup'){
    if(!name.trim()){toast.error('Please enter your name');setBusy(false);return}
   ...
   console.error(e);
   const messages={'auth/email-already-in-use':'An account with this email already exists','auth/invalid-email':'Please enter a valid email address','auth/invalid-credential':'Invalid email or password','auth/weak-password':'Password is too weak','auth/network-request-failed':'Network error. Please try again'};
 ...
 async function sendCode(e){
  ...
  if(!/^\+[1-9]\d{7,14}$/.test(clean)){toast.error('Enter your number with country code, e.g. +919876543210');return}
  if(!widgetReady||!window.sendOtp){toast.error('OTP service is still loading. Please try again in a moment.');return}
  setBusy(true);
  try{
   await new Promise((resolve,reject)=>{
    window.sendOtp(clean.slice(1),data=>{
     const id=data?.message||data?.reqId||data?.requestId;
     if(!id)reject(new Error('MSG91 did not return a request ID'));
     else{setReqId(String(id));resolve(data)}
    },error=>reject(new Error(error?.message||error?.type||'Could not send verification code')));
   });
 ...
 async function verifyCode(e){
  e.preventDefault();
  if(!confirmation||!reqId)return;
  if(!/^\d{6}$/.test(code)){toast.error('Enter all 6 digits of the verification code');return}
  setBusy(true);
  try{
   const result=await new Promise((resolve,reject)=>{window.verifyOtp(code,data=>resolve(data),error=>reject(new Error(error?.message||error?.type||'Incorrect or expired verification code')),reqId)});
   const token=typeof result==='string'?result:(result?.['access-token']||result?.accessToken||result?.token||result?.message);
   if(!token)throw new Error('MSG91 did not return a verification token');
   const signedIn=await finishPhoneSignIn(token);
   if(signedIn){toast.success('Welcome back to MotoDC!');resetOtp();setTimeout(()=>nav('/',{replace:true}),350)}
   else toast.success('New number verified. Enter your name to finish account setup.');
  }catch(e){
   console.error('MSG91 verify OTP error:',e);
 ...
 async function submitPhoneName(e){
  ...
  try{
   ...
   resetOtp();
   setTimeout(()=>nav('/',{replace:true}),350);
  }catch(e){console.error('MSG91 name setup error:',e);toast.error(e.message||'Could not create your account')}
  finally{setBusy(false)}
 }
 ...
 const signInForm=<form className="authform" onSubmit={submitEmail}>
  {mode==='signup'&&<label><span>Name</span><div><User size={17}/><input value={name} onChange={e=>setName(e.target.value)} placeholder="Your full name" autoComplete="name" required/></div></label>}
  <label><span>Email</span><div><Mail size={17}/><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email" required/></div></label>
  <label><span>Password</span><div><Lock size={17}/><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="At least 6 characters" autoComplete={mode==='signin'?'current-password':'new-password'} minLength="6" required/></div></label>
  <button className="heroBtn" disabled={busy}>{busy?(mode==='signin'?'Signing in...':'Creating account...'):(mode==='signin'?'Sign in':'Create account')}<ArrowRight size={17}/></button>
 </form>;

 const phoneNameForm=<form className="authform" onSubmit={submitPhoneName}>
  <label><span>Your name</span><div><User size={17}/><input value={name} onChange={e=>setName(e.target.value)} placeholder="Enter your full name" autoComplete="name" required autoFocus/></div></label>
  <p className="authhint"><ShieldCheck size={15}/> Mobile verified. Enter your name to create your MotoDC account.</p>
  <button className="heroBtn" disabled={busy}>{busy?'Creating account...':'Continue'}<ArrowRight size={17}/></button>
 </form>;

 const phoneForm=<form className="authform" onSubmit={sendCode}>
  <label><span>Phone number</span><div><Phone size={17}/><input type="tel" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="+91 9876543210" autoComplete="tel" required/></div></label>
  <p className="authhint"><ShieldCheck size={15}/> We'll send a verification code by SMS.</p>
  <button className="heroBtn" disabled={busy}>{busy?'Sending code...':'Send verification code'}<ArrowRight size={17}/></button>
 </form>;

 const otpForm=<form className="authform" onSubmit={verifyCode}>
  <label><span>6-digit verification code</span><div className="otpInputWrap"><ShieldCheck size={17}/><div className="otpBoxes" role="group" aria-label="6-digit verification code" onPaste={handleOtpPaste}>{Array.from({length:6},(_,index)=><input key={index} ref={el=>{otpRefs.current[index]=el}} className="otpBox" type="text" inputMode="numeric" pattern="[0-9]*" maxLength="1" value={otpDigits[index]} onChange={e=>handleOtpChange(index,e.target.value)} onKeyDown={e=>handleOtpKeyDown(index,e)} autoComplete={index===0?'one-time-code':'off'} aria-label={`OTP digit ${index+1}`} disabled={busy}/>)}</div></div></label>
  <p className="authhint">Code sent to <b>{phone}</b></p>
  <button className="heroBtn" disabled={busy}>{busy?'Verifying...':'Verify & continue'}<ArrowRight size={17}/></button>
  <button type="button" className="textBtn" onClick={()=>{setConfirmation(false);setOtpDigits(Array(6).fill(''));setCode('');setReqId('')}}>Use a different number</button>
 </form>;

 let methodForm=signInForm;
 if(method==='phone')methodForm=needsName?phoneNameForm:(confirmation?otpForm:phoneForm);

 return <section className="auth page"><div className="authcard">
