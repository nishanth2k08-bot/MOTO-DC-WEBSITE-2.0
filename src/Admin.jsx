...
import {collection,deleteDoc,doc,getDocs,addDoc,updateDoc,getDoc,setDoc} from 'firebase/firestore';
...
export default function Admin(){
 ...
 async function loadProducts(){try{const snap=await getDocs(collection(adminDb,'products'));const firestoreDocs=snap.docs.map(d=>({id:d.id,...d.data()})).filter(d=>!d.deleted);const byName=new Map();const byId=new Map();firestoreDocs.forEach(d=>{if(d.id)byId.set(d.id,d);if(d.name)byName.set(String(d.name).trim().toLowerCase(),d)});const merged=[...firestoreDocs];majorSpareProducts.forEach(sp=>{const norm=String(sp.name||'').trim().toLowerCase();if(!byId.has(sp.id)&&!byName.has(norm)){merged.push(sp)}});setProducts(merged)}catch(e){console.error(e);setProducts(majorSpareProducts);toast.error(e.code==='permission-denied'?'Admin permissions are not enabled yet':'Could not load products')}}
