...
import {collection,deleteDoc,doc,getDocs,addDoc,updateDoc,getDoc,setDoc,onSnapshot} from 'firebase/firestore';
...
import {mergeProductsWithMajorSpares} from './productsMerge';
import './admin.css';
...
export default function Admin(){
 ...
 useEffect(()=>{
    if(!allowed)return;
    const unsub=onSnapshot(collection(adminDb,'products'),snap=>{
      const firestoreDocs=snap.docs.map(d=>({id:d.id,...d.data()})).filter(d=>!d.deleted);
      setProducts(mergeProductsWithMajorSpares(firestoreDocs));
    },e=>{console.error(e);loadProducts();});
    return()=>unsub();
  },[allowed]);
  async function loadProducts(){try{const snap=await getDocs(collection(adminDb,'products'));const firestoreDocs=snap.docs.map(d=>({id:d.id,...d.data()})).filter(d=>!d.deleted);setProducts(mergeProductsWithMajorSpares(firestoreDocs))}catch(e){console.error(e);setProducts(mergeProductsWithMajorSpares([]));toast.error(e.code==='permission-denied'?'Admin permissions are not enabled yet':'Could not load products')}}
 ...
 async function save(e){e.preventDefault();if(!form.name.trim()||form.price===''||!form.image.trim()){toast.error('Name, price and product image are required');return}setSaving(true);const payload={name:form.name.trim(),category:form.category,brand:form.brand.trim(),fitment:form.fitment.trim(),price:num(form.price),rating:num(form.rating),stock:num(form.stock),image:form.image.trim(),description:form.description.trim(),adminUpdated:true,updatedAt:Date.now()};try{if(editing)await setDoc(doc(adminDb,'products',editing),payload,{merge:true});else await addDoc(collection(adminDb,'products'),payload);toast.success(editing?'Product updated':'Product added');reset();await loadProducts()}catch(e){console.error(e);toast.error('Save failed. Check Firebase admin rules.')}finally{setSaving(false)}}
 ...
 async function remove(id){if(!confirm('Delete this product permanently?'))return;try{await setDoc(doc(adminDb,'products',id),{deleted:true},{merge:true});await deleteDoc(doc(adminDb,'products',id)).catch(()=>{});setProducts(p=>p.filter(x=>x.id!==id));toast.success('Product deleted')}catch(e){console.error(e);toast.error('Delete failed. Check Firebase admin rules')}}
