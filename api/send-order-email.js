...
const requestRef=(db,requestCollection,id)=>db.collection('request').doc(String(requestCollection)).collection('requests').doc(String(id));
...
export default async function handler(req,res){
 ...
 try{
  ...
  const {event,orderDocId,returnId,requestCollection,paymentMethod}=req.body||{};if(!event)return send(res,400,{error:'Email event is required'});
  ...
  if(event==='cancelled'){
   ...
   if(!returnId||!requestCollection)return send(res,400,{error:'Return request reference is required'});
   requestRefValue=requestRef(db,requestCollection,returnId);const requestSnap=await requestRefValue.get();if(!requestSnap.exists)return send(res,404,{error:'Return request not found'});request={id:requestSnap.id,...requestSnap.data()};if(request.status!=='completed')return send(res,409,{error:'Return request is not completed'});if(!request.orderId)return send(res,400,{error:'Return request has no order reference'});
